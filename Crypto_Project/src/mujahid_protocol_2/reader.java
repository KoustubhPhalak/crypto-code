...
public class reader {
	...
	void calculateABC() {
		a = wrap.rotateLeft(wrap.rotateLeft(n1, Math.abs((IDS.xor(k1)).intValue())), Math.abs(k2.intValue()));
		b = wrap.rotateLeft(wrap.rotateLeft(n2, Math.abs((k2.xor(IDS)).intValue())), Math.abs((k1.xor(n1)).intValue()));
