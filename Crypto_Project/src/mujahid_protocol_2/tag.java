...
public class tag 
{
	...
	void calculaten1n2()
	{
		n1=wrap.rotateRight(wrap.rotateRight(a,Math.abs(k2.intValue())),Math.abs((IDS.xor(k1)).intValue()));
		//n2=wrap.rotateRight(wrap.rotateRight(b,Math.abs(k1.xor(n1)).intValue())),Math.abs((IDS.xor(k2)).intValue())).xor(k1);
		n2=wrap.rotateRight(wrap.rotateRight(b, Math.abs(k1.xor(n1).intValue())), Math.abs(IDS.xor(k2).intValue()));
